...
const oceanDescriptions = {
  ...
  en: {
    ...
    southern: "Surrounds Antarctica and is home to unique polar ecosystems.",
  },
};
...
const loginStrings = {
  ...
  en: {
    ...
    cryptoError: "Please run the site from a local server (HTTPS/localhost).",
    logout: "Logout",
  },
};
...
function initFeaturedOceans() {
  const container = document.getElementById("featured-oceans");
  if (!container) return;

  const descriptions = oceanDescriptions[currentLang];
  const names = oceanNames[currentLang];

  fetch("https://vebdizajn-4.onrender.com/api/vebdizajn/okeani")
    .then((res) => {
      if (!res.ok) throw new Error("API error");
      return res.json();
    })
    .then((data) => {
      const entries = Object.keys(data).slice(0, 3);

      entries.forEach((apiName) => {
        const enrichment = oceanData[apiName] || {};
        const displayName = names[apiName] || apiName;
        const description = descriptions[enrichment.key] || "";
        const imageUrl = enrichment.image || "";
